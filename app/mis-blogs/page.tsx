...
export default function MyBlogs() {
  ...
  const [blogsPaginationData, setBlogsPaginationData] = useState<number | null>(
    null
  );
  ...
  return (
    <Container sx={{ marginTop: "2rem", paddingBottom: 8 }}>
      ...
      {blogsPaginationData && Math.ceil(blogsPaginationData / 10) - 1 > 1 && (
        <ReactPaginate
          containerClassName="custom-pagination"
          activeClassName="p-1 px-2 border-customYellow-500 border rounded-lg"
          breakLabel="..."
          nextLabel={
            page >= Math.ceil(blogsPaginationData / 10) - 1 ? null : (
              <Button
                variant="contained"
                disabled={page >= Math.ceil(blogsPaginationData / 10) - 1}
                color="primary"
              >
                Siguiente
              </Button>
          ...
          previousLabel={
            page <= 0 ? null : (
              <Button variant="contained" disabled={page <= 0} color="primary">
                Anterior
              </Button>
            )
          }
